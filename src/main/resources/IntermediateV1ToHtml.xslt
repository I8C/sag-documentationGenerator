...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:tns="http://www.i8c.be/sag/documentationgenerator/intermediate/v1">
		...
		<html>
			...
			<body>
				<header>
					...
					<h2>webMethods Integration Server<br/>i8c generated Services Reference</h2>
			...
			<xsl:when test="..=/tns:Packages">
				<div class="package">
					...
					<div class="package-footer">
						<table>
							<tr>
								<td>WM IS i8c generated Services Reference Guide</td>
								<td>Version <xsl:value-of select="./@version"/></td>
							</tr>
						</table>
					</div>
				</div>
	...
	<xsl:template name="fixNewLines">
        ...
        <xsl:choose>
            <xsl:when test="not(contains($str, $newLineCharacter))">
                <xsl:copy-of select="$str"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:value-of select="substring-before($str, $newLineCharacter)"/>
                <br />
                <xsl:call-template name="fixNewLines">
                    <xsl:with-param name="str" select="substring-after($str, $newLineCharacter)"/>
                </xsl:call-template>
            </xsl:otherwise>
        </xsl:choose>
	</xsl:template>
