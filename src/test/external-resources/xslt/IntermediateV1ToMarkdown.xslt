...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:tns="http://www.i8c.be/sag/documentationgenerator/intermediate/v1">
	...
	<xsl:template match="/">
        ...
        <xsl:text>### i8c created Services Reference&#10;</xsl:text>
		...
		<xsl:choose>
			<xsl:when test="..=/tns:Packages">
				<div class="package">
					...
					<xsl:apply-templates/>
                    <xsl:text>&#10;&#10;</xsl:text>
                    <xsl:text>WM IS i8c created Services Reference Guide  &#10;</xsl:text>
                    <xsl:value-of select="concat('Version ', ./@version, '&#10;')"/>
				</div>
			</xsl:when>
			<xsl:otherwise>
	...
	<xsl:template name="Field" match="tns:Field">
		...
		<xsl:choose>
            <xsl:when test="./tns:Comment">
                <span class="field-comment"><xsl:value-of select="concat(', ', ./tns:Comment, '&#10;')"/></span>
            </xsl:when>
            <xsl:otherwise>
                <xsl:text>&#10;</xsl:text>
            </xsl:otherwise>
